<#
 ...
 .Description
  ...
  C:\<User>\GoogAPIAuth_client_id_userParam.txt
  C:\<User>\GoogAPIAuth_client_secret_userParam.txt
  C:\<User>\GoogAPIAuth_refresh_tokens_userParam.txt
...
Function Get-GooglePhotos(){
    ...
    param (
        ...
        [int]$Month,
        [Parameter(Mandatory=$false)]
        [string]$SavePath = "<YOUR_DEFAULT_FOLDER_PATH>"
    )
...
#    . "$($PSScriptRoot)\Get-GoogAPIAlbums.ps1"
    ...
    if(-not (Get-GoogApiAuth -user $AccountOwner -RefreshToken )){
        Get-GoogApiAuth -user $AccountOwner
    }
    ...
    #if(-not (Test-Path "$SavePath\Photos-001")){mkdir "$SavePath\Photos-001" -Force}

    #BUG SUR POWERSHELL --> DISABLE PROGRESSBAR IMPROVE DOWNLOAD SPEED -_-
    ...
    foreach($Media in $Contents){
        ...
        $err = 0
            ...
            if($Media.mimeType -match "image"){
                ...
                $FullSavePath =  "$SavePath\$($Media.filename)"

                if(-not (Test-Path $FullSavePath)){
                    do{
                    #$FullSavePath
                    #$DownloadURL
                    #BUG SUR POWERSHELL --> enlever la progress bar augmente la vitesse de téléchargement
                    $global:ProgressPreference = 'Stop'
                    Write-Output "$FullSavePath"
                    try {
                        $RawImage = $(Invoke-WebRequest -Uri $DownloadURL).Content
                        [System.IO.File]::WriteAllBytes($FullSavePath, $RawImage)
                        $success = $true  
                    }
                    catch {
                        $err = $err + 1
                        if($err -gt 6){
                            Write-Output "ERREUR SUR LE FICHIER : (DOWNLOAD LINK) -> $DownloadURL" | Out-File "$SavePath\LOGS.txt"
                            $success = $true
                        }
                        Write-Output "ERROR, RETRY IN 1 SEC -> COUNT = $err"
...
Function Get-GoogApiAuth (){
    ...
    $env:GoogAPIAuth_client_id_PATH = "$($env:USERPROFILE)\GoogAPIAuth_client_id$($user).txt"
    $env:GoogAPIAuth_client_secret_PATH = "$($env:USERPROFILE)\GoogAPIAuth_client_secret$($user).txt"
    $env:GoogAPIAuth_refresh_tokens_PATH = "$($env:USERPROFILE)\GoogAPIAuth_refresh_tokens$($user).txt"
